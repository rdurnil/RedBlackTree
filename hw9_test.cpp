...
//TEST 11: (1) Adding mirror case for AddRebalanceCheckLeftCases
...
//TEST 12: (2) Adding mirror case for RemoveRebalanceCheckLeftCases
...
//TEST 13: test keys
TEST(RBTCollectionTest, KeysTest) {
  RBTCollection<string,int> c;
  c.add("a", 1);
  c.add("b", 2);
  c.add("c", 3);
...
//TEST 14: Tests height and ensures that rebalance is occurring
TEST(RBTCollectionTest, HeightTest) {
  RBTCollection<string,int> c;
  c.add("a", 10);
...
//TEST 15: Tests add and removing values that don't exist
TEST(RBTCollectionTest, CantRemove) {
  RBTCollection<string,int> c;
  c.remove("a");
  ASSERT_EQ(0, c.size());
  c.add("a", 2);
...
//TEST 16: Tests remove, find, find range and sort on empty lists
TEST(RBTCollectionTest, EmptyLists) {
  RBTCollection<string,int> c;
  ASSERT_EQ(0, c.size());
  //test remove
  c.remove("a");
